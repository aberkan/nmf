...
> **Status:** Phases 1–4 implemented (tracking, popup UI, **10h / rolling-24h tab close**). Optional hardening is Phase 5 — see [IMPLEMENTATION_PLAN.md](./IMPLEMENTATION_PLAN.md).
...
- **Limit (Phase 4):** if **rolling last 24 hours &gt; 10 hours** (strict inequality) while the Facebook tab is the active tab in a focused Chrome window, **close** that tab. Re-checked on reconcile (tab/focus/URL changes) and on each **checkpoint** alarm so long sessions cannot skip the check.
